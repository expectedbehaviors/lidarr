{{- define "lidarr_config_xml_content" -}}
...
<Config>
  ...
  <InstanceName>{{ $configOptions.instanceName | default "Lidarr" }}</InstanceName>
  ...
  <PostgresMainDb>{{ $configXmlValues.postgresMainDb | default "lidarr-main" }}</PostgresMainDb>
  <PostgresLogDb>{{ $configXmlValues.postgresLogDb | default "lidarr-log" }}</PostgresLogDb>
  {{- end }}
